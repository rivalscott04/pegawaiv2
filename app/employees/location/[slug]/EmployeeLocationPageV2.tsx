...
export default function EmployeeLocationPageV2() {
	const canExport = useCanExportPegawai()
	const canEdit = useCanEditEmployees()

	const params = useParams<{ slug?: string }>()
	const slug = typeof params.slug === 'string' ? params.slug : ''
	const pathSegment = slug ? decodeURIComponent(slug) : ''
	const { sourceUnitSlug, fallbackSatkerInduk } = resolveLocationPathSegment(pathSegment)
	const canonicalInduk = sourceUnitSlug ? canonicalIndukFromSourceUnitSlug(sourceUnitSlug) : null
	const locationLabel =
		canonicalInduk != null
			? locationTitleFromCanonicalInduk(canonicalInduk)
			: pathSegment || 'Memuat...'
	...
	const isKanwilScope = Boolean(sourceUnitSlug?.trim().toLowerCase().startsWith('kanwil_'))

	const selectedSourceUnitLabel = useMemo(() => {
		if (!sourceUnitSlug) return ''
		const fromOpt = sourceUnitOptions.find((o) => o.value === sourceUnitSlug)?.label
		if (fromOpt) return fromOpt
		return formatSourceUnitSlug(sourceUnitSlug)
		...
		try {
			...
			const sourceUnitsFromSlug = Array.isArray(topLevelData?.source_unit_slug)
				? topLevelData.source_unit_slug
						...
						.map((s) => ({ value: s, label: formatSourceUnitSlug(s) }))
				: []
			...
			if (fallbackSatkerInduk) unitQs.set('satker_induk', fallbackSatkerInduk)
			if (sourceUnitSlug) unitQs.set('source_unit_slug', sourceUnitSlug)
			if (jenisPegawai) unitQs.set('jenis_pegawai', jenisPegawai)
	...
	const queryString = useMemo(() => {
		...
		if (sourceUnitSlug) p.set('source_unit_slug', sourceUnitSlug)
		if (fallbackSatkerInduk) p.set('satker_induk', fallbackSatkerInduk)
		if (effectiveUnitKerja) p.set('unit_kerja', effectiveUnitKerja)
		if (jenisPegawai) p.set('jenis_pegawai', jenisPegawai)
		if (pangkatGolongan) p.set('pangkat_golongan', pangkatGolongan)
	...
	async function load() {
		if (!sourceUnitSlug && !fallbackSatkerInduk) return
		setLoading(true)
		try {
			...
			console.error('load pegawai location v2 failed', e)
			setItems([])
		} finally {
			setLoading(false)
		}
	...
	useEffect(() => {
		void loadFilters()
	}, [sourceUnitSlug, fallbackSatkerInduk, jenisPegawai]) // eslint-disable-line react-hooks/exhaustive-deps -- loadFilters is stable enough for cascading filters
	...
	}, [queryString, sourceUnitSlug, fallbackSatkerInduk])
	...
	const columns = useMemo(() => {
		...
		if (isKanwilScope) return baseColumns
		...
		<div className="p-2 sm:p-4 md:p-8 overflow-x-hidden max-w-full">
			<div className="mb-4 space-y-2">
				<div className="breadcrumbs text-sm mb-2">
					<ul>
						<li>
							<a href="/heatmap">Sebaran Pegawai</a>
						</li>
						<li>{locationLabel}</li>
					</ul>
				</div>
				<div className="flex items-center justify-between flex-wrap gap-2">
					<div>
						<h1 className="text-2xl font-semibold">Pegawai di {locationLabel}</h1>
						<p className="text-sm opacity-70 mt-1">{canonicalInduk ?? fallbackSatkerInduk ?? ''}</p>
					</div>
					...
					{canExport && (
						<div className="dropdown dropdown-end">
							...
							<ul className="dropdown-content menu mt-1 w-72 rounded-box bg-[#001b3d] p-3 text-white shadow-xl z-50" role="menu">
								...
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('csv', 'page', 'comma')}>
										CSV (Koma ,)
									</button>
								</li>
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('csv', 'page', 'semicolon')}>
										CSV (Titik Koma ;)
									</button>
								</li>
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('xlsx', 'page')}>
										Excel (.xlsx)
									</button>
								</li>
								...
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('csv', 'all', 'comma')}>
										CSV (Koma ,)
									</button>
								</li>
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('csv', 'all', 'semicolon')}>
										CSV (Titik Koma ;)
									</button>
								</li>
								<li>
									<button disabled={exporting} onClick={() => exportFromBackend('xlsx', 'all')}>
										Excel (.xlsx)
									</button>
								</li>
							</ul>
			...
			<dialog id="pegawai_modal_v2_location" ref={detailModalRef} className="modal">
